...
def calculate_incident_heat_flux_from_controlled_fire_cfast(
        fire_mode: int,
        t_arr: np.ndarray,
    ...
    fire_area_2 = np.pi * (
            fire_travel_speed * np.where((_ := t_arr_ - q_x_d / (hrr_density_kWm2 * 1e3)) < 0, 0, _)) ** 2
...
def calculate_incident_heat_flux(
    ...
    if fire_mode == 0:
        ...
        q_1, phi_1, q_2, phi_2, t_d = calculate_incident_heat_flux_from_controlled_fire_cfast(
            fire_mode=fire_mode,
            t_arr=t,
