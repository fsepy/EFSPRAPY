...
if __name__ == '__main__':
    ...
    input_data = xlsx_to_dict(fp_input)
    for case_name in input_data.keys():
        ...
        try:
            shutil.rmtree(path.join(path.dirname(__file__), "cases", case_name))
        except FileNotFoundError:
            pass
        input_parser.to_cases_csv_and_json(path.join(path.dirname(__file__), "cases", case_name))
