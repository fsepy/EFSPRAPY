...
if __name__ == '__main__':
    ...
    kwargs_ = dict()
    for w, h in itertools.product((np.arange(0.2, 21.001, 0.4)), np.arange(0.2, 9.001, 0.4)):
    ...
    for w, h in itertools.product((np.arange(9, 21.001, 0.05)), np.arange(2, 6.001, 0.05)):
        kwargs_[f'{fp_xlsx.stem}-{w:06.3f}-{h:06.3f}'] = EXAMPLE_INPUT | dict(
            receiver_separation=sep_parallel_any_br187(w, h, 84, 12.6),
            room_width=w, room_height=h, opening_width=w, opening_height=h,
